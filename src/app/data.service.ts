...
getData(username): Observable<any> {
  ...
  const headers = {'content-type': 'application/json','Authorization' : `Bearer ${token}`};
    this.DataObservable = this.http.get('https://personal-budget-amulya-backend.herokuapp.com/budget',{ headers: headers,params:{userid : username }}).pipe(shareReplay());
...
private readonly NAMES = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'];
  ...
  userLogin(data:UserSchema){
    ...
    const body=JSON.stringify(data);
    console.log(body)
    return this.http.post('https://personal-budget-amulya-backend.herokuapp.com/auth',body,{'headers':headers}).subscribe((res:any)=>{
      console.log(res);       
